...
To integrate AWS Cognito and Amplify into your existing script, you'll need to add code for fetching resources and their tags from these services. Here’s how you can extend your script to include AWS Cognito and Amplify:

1. Import the Required Libraries
Add cognito-idp and amplify to your create_clients function.
...
2. Add Functions to Fetch Cognito and Amplify Resources
Here are the functions to fetch the Cognito User Pools, User Pool Clients, and Amplify Apps.
...
def get_cognito_user_pool_clients(client, user_pool_id):
    clients = paginate_boto3_results(client, 'list_user_pool_clients', 'UserPoolClients')
...
def get_amplify_apps(client):
    apps = paginate_boto3_results(client, 'list_apps', 'apps')
    for app in apps:
        tags = get_tags(client, 'amplify', app['appId'])
        yield {
            'ResourceType': 'Amplify App',
            'ResourceArn': f"arn:aws:amplify:{region}:{account_number}:apps/{app['appId']}",
            'ResourceName': app['name'],
            'Region': region,
            'Tags': tags
        }
3. Integrate Cognito and Amplify Fetching into the Main Script
Add the Cognito and Amplify resource fetching to your main loop.
...
for region in regions:
    ...
    # Existing resource fetching...

    # Fetch Cognito User Pools
    try:
        for user_pool in get_cognito_user_pools(clients['cognito-idp']):
            resources.append(user_pool)
    except ClientError as e:
        print(f"Error fetching Cognito User Pools: {e}")

    # Fetch Cognito User Pool Clients
    try:
        user_pools = paginate_boto3_results(clients['cognito-idp'], 'list_user_pools', 'UserPools')
        for pool in user_pools:
            for user_pool_client in get_cognito_user_pool_clients(clients['cognito-idp'], pool['Id']):
                resources.append(user_pool_client)
    except ClientError as e:
        print(f"Error fetching Cognito User Pool Clients: {e}")

    # Fetch Amplify Apps
    try:
        for app in get_amplify_apps(clients['amplify']):
...
Add Cognito and Amplify tags to your CSV headers if needed.

python
Copy code
csv_headers = ['Resource Type', 'Resource ARN', 'Region', 'Resource Name', 'Account Number'] + fiserv_tag_keys
This approach will seamlessly integrate AWS Cognito and Amplify into your existing script without disrupting its current functionality. Adjust the tag handling as needed if you have specific tags or key-value pairs for Cognito and Amplify resources.
...
Traceback (most recent call last):
  File "H:\pychram\pulled\newtest\vernewcopy.py", line 394, in <module>
    for user_pool in get_cognito_user_pools(clients['cognito-idp']):
  File "H:\pychram\pulled\newtest\vernewcopy.py", line 41, in get_cognito_user_pools
    user_pools = paginate_boto3_results(client, 'list_user_pools', 'UserPools')
                 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "H:\pychram\pulled\newtest\vernewcopy.py", line 36, in paginate_boto3_results
    for page in paginator.paginate():
